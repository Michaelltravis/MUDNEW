...
<head>
    ...
    <style>
        * {
        ...
        .send-btn:disabled, .quick-btn:disabled, .exit-btn:disabled, #command-input:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }
...
<body>
    ...
    <script>
        ...
        function setInteractiveDisabled(disabled, placeholderText) {
            ...
            document.querySelectorAll('.exit-btn').forEach(btn => btn.disabled = disabled);
        }
        ...
        function connect() {
            ...
            setInteractiveDisabled(true, 'Connecting...');
            const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
            ws = new WebSocket(`${protocol}//${location.host}/ws`);
            
            ws.onopen = () => {
                setStatus('', 'Connected');
                setInteractiveDisabled(false, 'Enter command...');
            ...
            ws.onclose = () => {
                ...
                setInteractiveDisabled(true, 'Disconnected');
