...
## 2024-10-25 - Custom Numeric Steppers Accessibility
**Learning:** Custom numeric steppers (like font size controls) fail to communicate their boundary limits to screen readers and visually to users if the `disabled` attribute isn't synced. Also, value changes aren't announced without `aria-live`.
**Action:** Synchronize the HTML `disabled` attribute with boundary limits and use `aria-live="polite"` on the display element to ensure screen readers announce value changes.
