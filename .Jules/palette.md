...
**Action:** Always ensure toggle buttons implement ARIA switch semantics.## 2026-04-07 - Disabling UI During Connection
**Learning:** Interactive input elements (like text inputs, send buttons, and quick command buttons) should be visually and programmatically disabled during WebSocket connection initialization and upon connection failure. Use CSS :not(:disabled) on hover/active pseudo-classes to prevent interactive visual effects on inactive elements.
**Action:** Use setInputState to toggle .disabled on interactive elements and update input placeholders.
