...
## 2026-03-23 - Interactive Element Connection States
**Learning:** Visually and programmatically disabling interactive input elements (like text inputs, send buttons, and quick command buttons) during WebSocket connection initialization and upon connection failure prevents user frustration and improves accessibility. It's important to use CSS `:not(:disabled)` on hover/active pseudo-classes to prevent interactive visual effects on inactive elements, and update input placeholders to explicitly explain the disabled state (e.g., 'Connecting...', 'Disconnected').
**Action:** Always ensure interactive elements are properly disabled and provide clear state feedback during connection flows.